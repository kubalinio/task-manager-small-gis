...
export class IndexDBError extends Error {
  ...
  constructor(message: string, code = 'INDEXDB_ERROR', status = 500, details?: Record<string, unknown>) {
    super(message);
...
export const createNotFoundError = (resource: string, id: string): IndexDBError => {
...
export const createValidationError = (message: string, details?: Record<string, unknown>): IndexDBError => {
  return new IndexDBError(
    message,
    'VALIDATION_ERROR',
    422,
